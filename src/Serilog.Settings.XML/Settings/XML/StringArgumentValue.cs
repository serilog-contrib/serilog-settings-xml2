...
namespace Serilog.Settings.XML
{
    internal class StringArgumentValue : IConfigurationArgumentValue
    {
        ...
        private static readonly Regex StaticMemberAccessorRegex = new("^(?<shortTypeName>[^:]+)::(?<memberName>[A-Za-z][A-Za-z0-9]*)(?<typeNameExtraQualifiers>[^:]*)$", RegexOptions.Compiled);
        ...
        private static readonly Dictionary<Type, Func<string, object>> ExtendedTypeConversions = new()
        {
        ...
        public object ConvertTo(Type toType, ResolutionContext resolutionContext)
        {
            ...
            if ((toTypeInfo.IsInterface || toTypeInfo.IsAbstract || toTypeInfo.IsAnsiClass) && !string.IsNullOrWhiteSpace(argumentValue))
            {
